...
def see_assigned_task(request):
    ...
    if filter_type and filter_value:
        if filter_type == 'title':
            tasks = tasks.filter(task_title__icontains=filter_value)
        elif filter_type == 'group':
            tasks = tasks.filter(Group__group_name__icontains=filter_value)
        elif filter_type == 'status':
            tasks = tasks.filter(task_status=filter_value)
        elif filter_type == 'created_at':
            tasks = tasks.filter(task_created_at=filter_value)  # YYYY-MM-DD format

    context = {
        'task_details': tasks
    }
    return render(request, "see_assigned_task.html", context)


# View for Searching Notification List (see_notification)
...
def see_notifications(request):
    ...
    if filter_type and filter_value:
        if filter_type == 'title':
            notifications = notifications.filter(title__icontains=filter_value)
        elif filter_type == 'type':
            notifications = notifications.filter(notify_type__icontains=filter_value)
        elif filter_type == 'link':
            notifications = notifications.filter(link__icontains=filter_value)
        elif filter_type == 'created_at':
            notifications = notifications.filter(notify_created_at__date=filter_value)  # YYYY-MM-DD format
